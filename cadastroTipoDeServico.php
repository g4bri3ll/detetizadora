...
<head>
    ...
    <title>Cadastrar o tipo de servico</title>
...
if (!empty($_SESSION)){
	foreach ($_SESSION['nome_paginas'] as $key) {
		switch ($key) {
			case 'cadastroTipoDeServico' : $as = 15;   break ;
		}
	}	
...
?>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Cadastro de tipo de servicos</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
									...
									if (!empty($_POST)){
										...
										if (empty($_POST['tipoServico'])){
											...
											if (empty($arrayVerificar)){
												...
												if ($result){
													?> <br><font size="3px" color="lime">Tipo de servicos cadastrado com sucesso! </font><br><br> <?php
												} else {
													?> <br><font size="3px" color="red"> Ocorreu um erro: <?php print_r($result); ?> </font><br><br> <?php
												}
												
											} else {
												?> <br><font size="3px" color="red"> Tipo de servico ja cadastrado com esse nome </font><br><br> <?php
											} 
            ...
            <?php 
            $tipDAO = new TipoServicoDAO();
            $arrayTipo = $tipDAO->listaTipoServicoAtivo();
            if (!empty($arrayTipo)){
            ?>
            ...
            if (!empty($_GET['exc'])){
            	...
            	$tsDAO->deleteId($id, $status);
            	
            	?> <script type="text/javascript"> window.location="cadastroTipoDeServico.php";  </script> <?php
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <?php foreach ($arrayTipo as $tipDAO => $valor) {?>
                                        <tr>
                                            <td><?php echo $valor['id']; ?></td>
                                            <td><?php echo $valor['nome']; ?></td>
                                            <td><?php echo $valor['status']; ?></td>
                                            <td><a href="cadastroTipoDeServico.php?exc=<?php echo $valor['id']; ?>"><i class="glyphicon glyphicon-trash"></i></a></td>
                                        </tr>
                                    <?php } ?>
                                    </tbody>
                                </table>
