...
<head>
    ...
    <title>Cadastro de pragas</title>
...
if (!empty($_SESSION)){
	foreach ($_SESSION['nome_paginas'] as $key) {
		switch ($key) {
			case 'cadastraPragas' : $as = 15;   break ;
		}
	}	
...
?>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Cadastro de pragas</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                                    ...
                                    <?php
										if (!empty($_POST)){
											...
											if (empty($_POST['nome'])){
												...
												$arrayVerificar = $praDAO->VerificaCad($nome);
														
												if (empty($arrayVerificar)){
													
													$praDAO = new PragasDAO();
													$result = $praDAO->cadastrar($nome, $status);
													
													if ($result){
														?> <font size="3px" color="lime">Praga cadastrado com sucesso</font><br><br><?php
													} else {
														?> <font size="3px" color="red"> Ocorreu um erro: <?php print_r($result); ?> </font> <?php
													}
													
												} else {
													?> <font size="3px" color="red"> Praga ja cadastrada </font><br><br> <?php
												} 
                                        ...
                                        <div class="form-group">
                                            <label>Informe o nome da praga</label>
                                            <input class="form-control" placeholder="Nome da praga" name="nome">
                                        </div>
            ...
            <?php 
            $praDAO = new PragasDAO();
            $arrayPragas = $praDAO->ListaPragas();
            ...
            if (!empty($_GET['exc'])){
            	...
            	$praDAO = new PragasDAO();
            	$praDAO->DeletaIdPraga($status, $id);
            	
            	?> <script type="text/javascript"> window.location="cadastraPragas.php";  </script> <?php
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <?php foreach ($arrayPragas as $praDAO => $valor) {?>
                                        <tr>
                                            <td><?php echo $valor['id']; ?></td>
                                            <td><?php echo $valor['nome_pragas']; ?></td>
                                            <td><?php echo $valor['status']; ?></td>
                                            <td><a href="cadastraPragas.php?exc=<?php echo $valor['id']; ?>"><i class="glyphicon glyphicon-trash"></i></a></td>
                                        </tr>
                                    <?php } ?>
                                    </tbody>
                                </table>
