...
class UploadIMGAdquacao{
		
	function imagem($foto, $idAtendeContrato){
		...
		if (isset ( $foto )) {
			...
			if (!empty($name)) {
				...
				if($size > $tamanho) {
					$error[4] = "A imagem deve ter no maximo ".$tamanho." bytes"; //$size;
				}
				...
				if (empty($error)) {
					...
					$caminho_imagem = "C://xampp//htdocs//ProjetoIgor//Img//chamados_atendidos//" . $nome_imagem;
							
					// Faz o upload da imagem para seu respectivo caminho
					move_uploaded_file($tmp_name, $caminho_imagem);
					
					$conDAO = new ContratoDAO();
					$conDAO->CadastrarFotosAtendidaContratos($nome_imagem, $idAtendeContrato);
